...
# Kubernetes setup with fluxcd

# mutual TLS -> Linkerd 

Have communication encrypted between application pods.

For 2 identities, mutual means that each part verifies the tls cert of the other part.

# CI with self-hosted internal Argo Workflows

## github.com code <- https outbound <- k8s cronjob poller -> new commit -> argo workflow ...

## OR just register github runners as you need ONLY outbound connection from runner to github.com 

That's because the github runner process does:

1. infinite https long-poll requests to github.com, when github.com has a job for runner it responds to the https request with the job content
2. github runner sends a websocket request to github.com to send in real-time the status of job

You neeed a PAT token or something similar with admin permission on a repo to ask for a registration token for runner in that repo. 

A github runner needs to be registered first, the registration token is valid for 1h. If you would be able to set registration token to 1 year, the registration token would let someone keep creating new footholds indefinitely. 

Once, they are registered, they get .credentials.
...
What they CANNOT do with just this key (.credentials):
...
buildkit or kaniko for multi-arch, maybe somthing docker-bake style?) and push artifacts
...
notify incidents, k8s cronjob poller fails, etc. 
